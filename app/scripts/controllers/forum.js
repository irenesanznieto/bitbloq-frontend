(function() {
    ...
    angular.module('bitbloqApp')
        .controller('ForumCtrl', function(forumApi, $route, tokenApi, alertsService, ngDialog, $routeParams, $location, $scope, $rootScope, $log, utils, imageApi, $q, textAngularManager, _, $timeout, userApi) {
            var forum = this;
            forum.iSaySeed = true;
            ...
            forum.banUser = function(user) {
                if (user.id !== $scope.common.user.id) {
                    userApi.setUserBanned({id: user.id}, true).then(function() {
            ...
            forum.allowUser = function(user) {
                userApi.setUserBanned({id: user.id}, false).then(function() {
                ...
                forumApi.postAnswer(answer).then(function(response) {
                    ...
                    if (forum.answer.images.length > 0) {
                        ...
                        forum.answer.images.forEach(function(value, index) {
                            ...
                            images.push({id: imageId, type: value.type});
            ...
            function goForumTheme(themeId, themeCategory) {
                forumApi.getTheme(themeId).then(function(response) {
                    if ($scope.common.user && utils.isNewVisit(response.data, $scope.common.user.id)) {
                        forumApi.addViewer(themeId);
                    }
                                ...
                                if (answer.content.indexOf('<img') > -1) {
                                    ...
                                    for (var i = 0; i < images.length; i++) {
                                        var img = images[i],
                                            ...
                                            imgType = _.find(answer.images, {id: imgId}).type;
