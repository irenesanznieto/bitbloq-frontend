...
module.exports = function(grunt) {
    ...
    grunt.registerTask('properties', 'create properties script', function() {
        var fs = require('fs');

        var originPath = 'Properties/',
            hiddenFilePattern = /^\./,
            content,
            tempFile;
        ...
        folders.forEach(function(file) {
            if (!hiddenFilePattern.test(file)) {
                tempFile = grunt.file.readJSON(originPath + file);
                content = 'db.bitbloq_Properties.remove({});\ndb.bitbloq_Properties.insert(\n';
                content += JSON.stringify(tempFile);
                content += ');';
                grunt.file.write('propertiesScripts/script_' + file, content);
            }
        });
    ...
    grunt.registerTask('joinscripts', 'join all scripts', function(version) {
        var gruntTaskDone = this.async();
        //get bloqs

        var https = require('https');

        https.get('https://raw.githubusercontent.com/bq/bloqs/master/dist/script.json').on('response', function(response) {
            var body = '';
            var i = 0;
            ...
            response.on('end', function() {
                if (body.length) {
                    grunt.file.write('.tmp/bloqs.js', body);
                } else {
                ...
                finalScript += 'var conn = connect("localhost:27017/admin");\nconn.auth(\'admin\', \'******\');\ndb = conn.getSiblingDB(\'resmi\');\n\n';

                finalScript += grunt.file.read('changelogs/script.js');
                finalScript += '\n\n';
                finalScript += grunt.file.read('propertiesScripts/script_properties.json');
                finalScript += '\n\n';
                finalScript += grunt.file.read('faqs/script.js');
                finalScript += '\n\n';
                finalScript += grunt.file.read('.tmp/bloqs.js');
                finalScript += '\n\n';

                grunt.file.write('contentScripts/content_script_' + version + '.js', finalScript);
    ...
    grunt.registerTask('script', 'create content script', function(version) {

        if (!version) {
            grunt.log.error('Tienes que indicar la version, grunt script:210');
        } else {
            grunt.task.run([
                'changelog',
                'properties',
